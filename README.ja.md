...
> **📋 APIコントラクト**: 公開APIのコード例とコントラクトテストについては[README.md](README.md)の「Public API Contract」セクションを参照してください。`tests/test_readme.py`がすべてのコントラクトを検証します。
...
# 端末でログを表示
logler view app.log
logler view app.log --level ERROR

# 調査（相関ID / トレースID）
logler investigate app.log --correlation req-123
logler investigate app.log --trace trace-abc123
logler investigate app.log --correlation req-123 --hierarchy --waterfall

# LLM向けCLI（JSON出力がデフォルト）
logler llm schema app.log
logler llm search app.log --query "db"
