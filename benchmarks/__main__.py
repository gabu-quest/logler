...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    print("\n  logler benchmark suite")
    print(f"  Scale: {scale.name} (entries: {', '.join(f'{n:,}' for n in scale.entry_counts)})")
...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    for suite_name, scenarios in ALL_SUITES.items():
        ...
        for s in scenarios:
            ...
            total += 1
        print()
    print(f"  Total: {total} scenarios across {len(ALL_SUITES)} suites\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="benchmarks",
        description="logler benchmark suite — real benchmarks using only public APIs",
    )
