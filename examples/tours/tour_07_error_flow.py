...
def _():
    ...
    print(f"Root cause: Database connection timeout")
    print(f"Additional failure: ML model timeout")
...
def _(follow_thread_hierarchy, log_file):
    ...
    hierarchy = follow_thread_hierarchy(
        files=[str(log_file)],
        root_identifier="trace-error-001"
    )

    print(f"=== Hierarchy Overview ===")
...
def _(error_analysis):
    ...
    for _chain in error_analysis['propagation_chains']:
        ...
        for _node in _chain['chain']:
            _indent = "  " * _node['depth']
            print(f"  {_indent}↳ {_node['node_id']} (depth={_node['depth']}, errors={_node['error_count']})")
        print()
    return
