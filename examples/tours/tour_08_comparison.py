...
def _():
    ...
    before_logs.append(
        {
            ...
            "trace_id": "trace-before",
            "span_id": "users.db",
            "parent_span_id": "users.request",
            "operation_name": "Users DB Query",
            "service": "postgres",
            "duration_ms": 50,
        }
    )
    ...
    before_logs.append(
        {
            "timestamp": (base_time + timedelta(milliseconds=70)).isoformat(),
            "level": "INFO",
            "message": "Cache lookup",
            "trace_id": "trace-before",
            "span_id": "users.cache",
            "parent_span_id": "users.request",
            "operation_name": "User Cache Lookup",
            "service": "redis",
            "duration_ms": 10,
        }
    )
    ...
    before_logs.append(
        {
            "timestamp": (base_time + timedelta(milliseconds=90)).isoformat(),
            "level": "INFO",
            "message": "Response formatting",
            "trace_id": "trace-before",
            "span_id": "response.format",
            "parent_span_id": "users.request",
            "operation_name": "Format Response",
            "service": "api",
            "duration_ms": 20,
        }
    )
    ...
    after_logs.append(
        {
            "timestamp": (base_time + timedelta(seconds=60, milliseconds=0)).isoformat(),
            "level": "INFO",
            "message": "HTTP GET /api/users",
    ...
    after_logs.append(
        {
            "timestamp": (base_time + timedelta(seconds=60, milliseconds=10)).isoformat(),
            "level": "INFO",
            "message": "Database query",
            "trace_id": "trace-after",
            "span_id": "users.db",
    ...
    after_logs.append(
        {
            "timestamp": (base_time + timedelta(seconds=60, milliseconds=420)).isoformat(),
            "level": "WARN",
            "message": "Cache miss",
            "trace_id": "trace-after",
            "span_id": "users.cache",
    ...
    after_logs.append(
        {
            "timestamp": (base_time + timedelta(seconds=60, milliseconds=450)).isoformat(),
            "level": "INFO",
            "message": "Response formatting",
            "trace_id": "trace-after",
            "span_id": "response.format",
    ...
    after_logs.append(
        {
            "timestamp": (base_time + timedelta(seconds=60, milliseconds=480)).isoformat(),
            "level": "INFO",
            "message": "Audit logging",
            "trace_id": "trace-after",
            "span_id": "audit.log",
            "parent_span_id": "users.request",
            "operation_name": "Audit Log",
    ...
    return after_file, before_file, temp_dir
...
def _():
    ...
    _compare_logs.extend(
        [
            {
                "timestamp": (_base + _td(milliseconds=0)).isoformat(),
                "level": "INFO",
                "message": "Request started",
                "correlation_id": "req-success",
            },
            {
                "timestamp": (_base + _td(milliseconds=10)).isoformat(),
                "level": "INFO",
                "message": "Cache hit",
                "correlation_id": "req-success",
            },
            {
                "timestamp": (_base + _td(milliseconds=20)).isoformat(),
                "level": "INFO",
    ...
    _compare_logs.extend(
        [
            {
                "timestamp": (_base + _td(milliseconds=100)).isoformat(),
                "level": "INFO",
                "message": "Request started",
                "correlation_id": "req-failed",
            },
            {
                "timestamp": (_base + _td(milliseconds=110)).isoformat(),
                "level": "WARN",
                "message": "Cache miss",
                "correlation_id": "req-failed",
            },
            {
                "timestamp": (_base + _td(milliseconds=120)).isoformat(),
                "level": "INFO",
                "message": "Database query",
                "correlation_id": "req-failed",
            },
            {
                "timestamp": (_base + _td(milliseconds=500)).isoformat(),
                "level": "ERROR",
                "message": "Database timeout",
                "correlation_id": "req-failed",
            },
            {
                "timestamp": (_base + _td(milliseconds=510)).isoformat(),
                "level": "ERROR",
...
@app.cell
def _(cross_service_timeline, temp_dir):
    import json as _json3
    from pathlib import Path as _Path3
    from datetime import datetime as _dt3, timezone as _tz3, timedelta as _td3

    _base = _dt3(2024, 1, 15, 12, 0, 0, tzinfo=_tz3.utc)
    _cid = "req-compare-001"

    api_log = _Path3(temp_dir) / "compare_api.log"
    db_log = _Path3(temp_dir) / "compare_db.log"
    cache_log = _Path3(temp_dir) / "compare_cache.log"

    api_entries = [
        {
            "timestamp": (_base + _td3(milliseconds=0)).isoformat(),
            "level": "INFO",
            "message": "Received GET /api/users",
            "service": "api",
            "correlation_id": _cid,
        },
        {
            "timestamp": (_base + _td3(milliseconds=120)).isoformat(),
            "level": "INFO",
            "message": "User response serialized",
            "service": "api",
            "correlation_id": _cid,
        },
    ]
    ...
    db_entries = [
        {
            "timestamp": (_base + _td3(milliseconds=30)).isoformat(),
            "level": "INFO",
            "message": "DB query started",
            "service": "postgres",
            "correlation_id": _cid,
        },
        {
            "timestamp": (_base + _td3(milliseconds=95)).isoformat(),
            "level": "INFO",
            "message": "DB query finished (65ms)",
            "service": "postgres",
            "correlation_id": _cid,
        },
    ]
    ...
    cache_entries = [
        {
            "timestamp": (_base + _td3(milliseconds=40)).isoformat(),
            "level": "INFO",
            "message": "Cache lookup miss",
            "service": "redis",
            "correlation_id": _cid,
        }
    ]

    for _path, _entries in (
        (api_log, api_entries),
        (db_log, db_entries),
        (cache_log, cache_entries),
    ):
        with open(_path, "w") as _f:
            for _entry in _entries:
                _f.write(_json3.dumps(_entry) + "\n")

    timeline = cross_service_timeline(
        files={"api": [str(api_log)], "db": [str(db_log)], "cache": [str(cache_log)]},
        correlation_id=_cid,
    )

    print("=== Cross-Service Timeline ===")
    print(f"Services: {timeline.get('services', [])}")
    print(f"Total entries: {timeline.get('total_entries', 0)}")
    print(f"Duration: {timeline.get('duration_ms', 0)}ms\n")

    for _event in timeline.get("timeline", [])[:10]:
        _svc = _event.get("service", "unknown")
        _rel = _event.get("relative_time_ms", 0)
        _msg = _event.get("entry", {}).get("message", "")
        print(f"+{_rel:4d}ms [{_svc:8}] {_msg}")
    return
