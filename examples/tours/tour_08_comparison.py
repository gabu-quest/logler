...
def _():
    ...
    return compare_threads, compare_time_periods, cross_service_timeline, diff_hierarchies, follow_thread_hierarchy, format_hierarchy_diff
...
def _(after_file, before_file, follow_thread_hierarchy):
    ...
    hierarchy_before = follow_thread_hierarchy(
        files=[str(before_file)],
        root_identifier="trace-before"
    )

    hierarchy_after = follow_thread_hierarchy(
        files=[str(after_file)],
        root_identifier="trace-after"
    )

    print(f"Before: {hierarchy_before.get('total_duration_ms', 0)}ms, {hierarchy_before['total_nodes']} nodes")
    print(f"After: {hierarchy_after.get('total_duration_ms', 0)}ms, {hierarchy_after['total_nodes']} nodes")
...
def _(diff_hierarchies, hierarchy_after, hierarchy_before):
    ...
    diff = diff_hierarchies(
        hierarchy_before,
        hierarchy_after,
        label_a="Before Deploy",
        label_b="After Deploy"
    )

    print("=== Hierarchy Comparison ===\n")
    _summary = diff['summary']
    print(f"Duration change: {_summary['total_duration_change_ms']:+.0f}ms ({_summary['total_duration_change_pct']:+.1f}%)")
...
def _():
    ...
    _compare_logs.extend([
        {"timestamp": (_base + _td(ms=0)).isoformat(), "level": "INFO",
         "message": "Request started", "correlation_id": "req-success"},
        {"timestamp": (_base + _td(ms=10)).isoformat(), "level": "INFO",
         "message": "Cache hit", "correlation_id": "req-success"},
        {"timestamp": (_base + _td(ms=20)).isoformat(), "level": "INFO",
         "message": "Response sent", "correlation_id": "req-success"},
    ])

    # Failed request (req-failed)
    _compare_logs.extend([
        {"timestamp": (_base + _td(ms=100)).isoformat(), "level": "INFO",
         "message": "Request started", "correlation_id": "req-failed"},
        {"timestamp": (_base + _td(ms=110)).isoformat(), "level": "WARN",
         "message": "Cache miss", "correlation_id": "req-failed"},
        {"timestamp": (_base + _td(ms=120)).isoformat(), "level": "INFO",
         "message": "Database query", "correlation_id": "req-failed"},
        {"timestamp": (_base + _td(ms=500)).isoformat(), "level": "ERROR",
         "message": "Database timeout", "correlation_id": "req-failed"},
        {"timestamp": (_base + _td(ms=510)).isoformat(), "level": "ERROR",
         "message": "Request failed", "correlation_id": "req-failed"},
    ])
    ...
    print(f"Created compare.log with successful and failed requests")
    return (compare_file,)
...
def _(compare_file, compare_threads):
    ...
    thread_comparison = compare_threads(
        files=[str(compare_file)],
        correlation_a="req-success",
        correlation_b="req-failed"
    )
...
def _():
    ...
    print(f"Created periods.log spanning 14:00-16:00")
    return (period_file,)
