...
def _(mo):
    mo.md(
        r"""
    # Tour 12: Multi-File Interleaving - The Real Power

    **This is where Logler becomes godlike.**

    Most log tools work with single files. But real distributed systems have logs
    scattered across dozens of services, each in their own file. Logler can:

    - Load ALL files into a single index
    - Search across all files with ONE query
    - Trace a request as it flows through 5, 10, 20 services
...
def _(all_files):
    ...
    return Investigator, inv, meta, total_lines
...
def _(mo):
    mo.md(
        r"""
    ## 4. Cross-Service Timeline - THE KILLER FEATURE

    This is what makes Logler special: `cross_service_timeline()` creates
    a unified view of a request across ALL services with relative timings.

    Watch the request flow through each service in order!
    """
    )
    return
...
def _(
    ...
    for _svc, _count in timeline.get("service_breakdown", {}).items():
        print(f"  {_svc}: {_count}")
    return cross_service_timeline, timeline
...
def _(CORRELATION_ID, all_files):
    ...
    return follow_thread_hierarchy, get_hierarchy_summary, hierarchy
...
def _(hierarchy):
    ...
    return analyze_error_flow, error_flow, format_error_flow
...
def _(mo):
    ...
    This is how you debug microservices like a god.
